...
df_FH <- Rating_Cor(Data = paste0(dir_Work, "/Data/df_behav_.csv"),
                    Cond = "A",
                    SceneBreaks = c(0, 189, 241, 243, 419,
                                   431, 521, 527, 581, 625,
                                   757, 797, 827, 877, 885,
...
df_SH <- Rating_Cor(Data = paste0(dir_Work, "/Data/df_behav_.csv"),
                    Cond = "B",
                    SceneBreaks = c(0, 14, 28, 75, 158,
                                   186, 279, 283, 335,
                                   398, 405, 681, 877,
...
# Binding the dataframes together for First and Second Half Raters
df_rating_cor <- rbind(df_FH,
                       df_SH)

# Name the columns for clarity
names(df_rating_cor) <- c("PID_2", "PID_1", "Rating_Correlation")
...
m1 <- lmer(Recall_Correlation ~ Rating_Correlation * Theory + (1|PID_1) + (1|PID_2), data = df_cor, REML = F)
summary(m1)
```

```{r Specifying a Comparison Model for Comparison}
m2 <- lmer(Recall_Correlation ~ Rating_Correlation + Theory + (1|PID_1) + (1|PID_2), data = df_cor, REML = F)
anova(m2,m1)
```
