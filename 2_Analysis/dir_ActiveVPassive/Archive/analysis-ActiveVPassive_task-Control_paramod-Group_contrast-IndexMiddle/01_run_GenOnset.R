...
for (INDEX in 1:length(files)) {
  ...
  df_behav <- rucleaner(file = files[INDEX],
                       dir = dir,
                       unit_secs = NA,
                       shave_secs = 0)
...
df_behav <- subset(df,
  ...
  write.table(df_behav, paste0(here(), "/../data/control_index_vols-210.txt"), 
              sep = "\t", col.names = F, row.names = F)
  df_behav$Interval <- df_behav$Interval + 30
  df_behav <- subset(df_behav,select = c("Interval", "Dur", "TotalButtonPresses"))
  write.table(df_behav, paste0(here(), "/../data/control_index_vols-240.txt"), 
              sep = "\t", col.names = F, row.names = F)
...
df_behav <- subset(df,
  ...
  df_behav$Dur <- 2
  df_behav <- subset(df_behav,select = c("Interval", "Dur", "TotalButtonPresses"))
  write.table(df_behav, paste0(here(), "/../data/control_middle_vols-210.txt"), 
              sep = "\t", col.names = F, row.names = F)
  df_behav$Interval <- df_behav$Interval + 30
  df_behav <- subset(df_behav,select = c("Interval", "Dur", "TotalButtonPresses"))
  write.table(df_behav, paste0(here(), "/../data/control_middle_vols-240.txt"), 
              sep = "\t", col.names = F, row.names = F)
