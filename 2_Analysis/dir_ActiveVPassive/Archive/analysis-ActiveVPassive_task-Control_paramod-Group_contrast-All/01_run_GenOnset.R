...
write.table(df_behav, paste0(here(), "/../data/deriv/pipeline_1/fmriprep/group_onsets/task-Control_contrast-All_vols-210.txt"), 
            sep = "\t", col.names = F, row.names = F)
df_behav$Interval <- df_behav$Interval + 30
df_behav <- subset(df_behav,select = c("Interval", "Dur", "TotalButtonPresses"))
write.table(df_behav, paste0(here(), "/../data/deriv/pipeline_1/fmriprep/group_onsets/task-Control_contrast-All_vols-240.txt"), 
            sep = "\t", col.names = F, row.names = F)
